...
/**
 * Clase para definir el controlador de los botones de PanelAdministrar
 * 
		...
		if (e.getActionCommand().equals("Aceptar Matricula")) {
			...
			try {
				if (ventana.getSistema().aceptarMatricula(panel.getMatricula()) == false) {
					JOptionPane.showMessageDialog(panel, "Error al aceptar la matricula", "Error",
							JOptionPane.ERROR_MESSAGE);
					return;
				} else {
					JOptionPane.showMessageDialog(panel, "La peticion de matricula ha sido aceptada",
							"Aceptar matricula", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
			...
			try {
				if (ventana.getSistema().denegarMatricula(panel.getMatricula()) == false) {
					JOptionPane.showMessageDialog(panel, "Error al denegar la matricula", "Error",
							JOptionPane.ERROR_MESSAGE);
					return;
				} else {
					JOptionPane.showMessageDialog(panel, "La peticion de matricula ha sido denegada",
							"Denegar matricula", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
			...
			if (ventana.getSistema().agregarExpulsion(panel.getAlumnoAsignaturas(),
					panel.getAsignaturaAsig()) == false) {
				...
				try {
					if (ventana.getSistema().expulsarAlumno(panel.getExpulsion()) == false) {
						JOptionPane.showMessageDialog(panel, "Error al expulsar al alumno", "Error",
								JOptionPane.ERROR_MESSAGE);
						return;
					} else {
						JOptionPane.showMessageDialog(panel,
								"El alumno " + panel.getAlumnoAsignaturas().getNombre() + " ha sido expulsado de "
										+ panel.getAsignaturaAsig().getNombre(),
								"Expulsar", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
			...
			if (ventana.getSistema().comprobarExpulsion(panel.getAlumnoExpulsion(), panel.getAsignaturaExp(),
					panel.getExpulsionVigente()) != false) {
				JOptionPane.showMessageDialog(panel, "Error al readmitir al alumno", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			} else {
				...
				JOptionPane
						.showMessageDialog(panel,
								"El alumno " + panel.getAlumnoExpulsion().getNombre() + " ha sido readmitido en "
										+ panel.getAsignaturaExp().getNombre(),
								"Readmitir", JOptionPane.INFORMATION_MESSAGE);
				return;
